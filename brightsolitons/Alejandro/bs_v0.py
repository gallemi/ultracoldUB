...
ww = 0.5*Zmax        # walls width as a %1 of the grid
wh = 0.0      #walls height
bx = 0.5*Zmax      #barrier position a %1 of the grid
bw = 0.05*Zmax          #barrier width a %1 of the grid
bh = 0.0       #barrier heigh
a_s = -0.025        # scattering length 
#whoz = 1.0       # harmonic oscilator angular frequency
x0 = 0.0            #soliton initial position
v = -3.0           #inital push
...
Dtr = 1.0e-3*20          # real time step
Dti = 1.0e-3*20           # imaginary time step
#
...
#
# initial wf: bright_soliton(x,n,x0,gn,v): # 1/cosh(x/\xi)
# Bright Soliton 
c0=normaliza(bright_soliton(zp,Npoint,x0,gint,0)) # wf at t=0
...
Vpot_R = changeFFTposition(potencial(0),Npoint,1)     # prepare for fftpotencial(0)
t0=0.0 
...
plt.plot(z, potencial(0), 'g-') # plot the box
...
Vpot_R = changeFFTposition(potencial(1),Npoint,1)     # prepare for fft     # Add the barrier. # Prepare for fft
t0=0.0 
...
psi = changeFFTposition(cc,Npoint,0) # psi is the final wave function
plt.plot(z, abs(psi)**2, 'r-',label='$|\psi|^2$') # plot initial density
plt.plot(z, potencial(1), 'g-') # plot the box
...
#figure window
fig = plt.figure()
ax = plt.axes(xlim=(-Zmax, Zmax), ylim=(0, 1))
line, = ax.plot([], [], lw=2)
c=c0
cc = ifft(c)*Npoint*NormWF**0.5 # FFT from K3 to R3 and include the wf norm
psi = changeFFTposition(cc,Npoint,0) # psi is the final wave function
